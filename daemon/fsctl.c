    ...
    nfs41_upcall *upcall)
{
    ...
    if (args->outputformat == FILE_NFS41_QUERY_IDMAP_FORMAT_CPV) {
        ...
        snprintf_len = snprintf(outbuffer, outbuffersize,
            ...
            "idmapdata_local_username='%s'\n"
            "idmapdata_local_groupname='%s'\n"
            "idmapdata_local_uid=%ld\n"
            "idmapdata_local_gid=%ld\n"
            "idmapdata_nfs_username='%s'\n"
            "idmapdata_nfs_groupname='%s'\n"
            "idmapdata_nfs_uid=%ld\n"
            "idmapdata_nfs_gid=%ld\n",
            state->path.path,
            (long long)info.fileid,
            (long long)info.fsid.major,
            (long long)info.fsid.minor,
